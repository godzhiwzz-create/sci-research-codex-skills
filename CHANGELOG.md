...
- 修正主页核心定位：前置研究目标、假设比较、已授权实验推进与结果决策；精读演示降为辅助模块示例，同步 README、封面与项目教程，不改动 Skill 执行规则。
- 将 README 与 GitHub Pages 改为科研需求导向：前置使用问题、具体产出、精读成品和开始入口；版本号与更新历史留在专门文档，并同步维护规则与回归测试。
- 重新设计 GitHub Pages 与 README 首屏：统一深色科研视觉、双职责关系图、8 模块导航、分级工作流、真实精读演示和安全安装区；提供可退化的命令复制、键盘焦点和移动端布局，无新增第三方运行依赖或追踪。
