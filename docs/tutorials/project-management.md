...
本教程介绍如何围绕研究目标接续课题、推进已授权工作，并根据证据决定下一步。状态查询、只读核验与自主研究是不同范围，不要把其中一种用法套在所有任务上。

## 1. 从当前任务进入

| 任务 | 入口 | 何时结束 |
|---|---|---|
| 在已有授权内推进课题 | `sci-research-manager` | 完成当前有决策价值的阶段，核对结果并接续下一动作；触及授权边界或真正依赖外部条件时报告 |
...
根据当前研究目标和已有证据比较路线，在已有授权内推进下一步，
核对结果并更新判断。需要我作重大取舍时，给出具体建议和依据。
```

用户确定目标、重大取舍与授权范围。研究负责人承担假设比较、证据整合与下一步选择；长期执行仍需可用的运行环境与等待机制，不因安装 Skill 自动发生。下文的只读示例用于限制特定请求，不是所有研究工作的默认终点。
