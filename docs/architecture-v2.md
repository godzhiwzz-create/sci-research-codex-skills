...
owner 是当前主执行者承担的职责，不等于一个额外代理或强制 Skill 调用。专门入口可直接完成用户的窄任务；只有跨库状态集成、证据晋升、方向或稿件阶段改变，才需要对应 owner 的决策。缺少可选模块时使用保持验证和权限边界的可用能力，或说明具体缺口，不自动安装依赖。
...
文献到实验：接受新方法、改变信号用途或扩大昂贵投入前，先检查最近先例和可检验差别，分开论文证据与项目假设。复用仍适用的 brief 或既有记录中的紧凑段落；接受并授权具有可证伪要求的实验后才分配 ID、冻结变量、控制、混杂和运行门。普通阅读与讨论不自动产生实验卡。
...
局部改动只读取和更新受影响的内容；完成一段编辑不等于阶段转换，不要求重新建立整套阶段产物。完整阶段收尾和最终提交审核只在用户要求的对应任务范围内触发；有效的既有授权不因切换 Skill 而失效。
